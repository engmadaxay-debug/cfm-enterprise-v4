...
router.get('/', asyncHandler(async (req, res) => {
  ...
  const [people, vaults, journal, closing, profit, activities] = await Promise.all([
    ...
    pool.query(`SELECT COALESCE(SUM(profit_amount),0)::numeric(18,2) total_profit FROM exchange_profit WHERE created_at::date >= date_trunc('month', CURRENT_DATE)::date ${ownerAnd}`, params),
